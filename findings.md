software-as-a-besides	Software as a Besides	Type. The computer shows. Software that teaches you to not need it.	besides SaaB intent translation subtract shell kernel literacy
lookdown	Lookdown	Pattern matching intent. One file answers from disk. File shrinks as the adapter	lookdown TSV pattern-matching intent routing adapter pruning subtract
variance-lab	Variance Lab	28 models. 100k+ inferences. N=100. Cross-platform. Findings from running models	variance-lab empirical inference local-LLM measurement methodology N=100 cross-platform
inspiration	Inspiration	The people whose work subtract depends on. Ritchie, Fox, Korn, Ramey, McIlroy, T	lineage inspiration C bash ksh ssh sshsig llama.cpp ggml subtract dependencies
lineage	Lineage	The people whose code subtract depends on. Fox, Ramey, Korn, McIlroy, Thompson, 	lineage bash ksh ssh sshsig llama.cpp coprocesses pipes grep subtract dependencies
emile	Emile	Local-first deployment pattern for sole proprietors. One device, one model, one 	emile deployment local-first sole-proprietor Pi5 Mac-Mini clinical signing substrate
hosuni	Hosuni	Shell dispatcher. Routes intent through subtract, lookdown, and inference. 68000	hosuni dispatcher shell routing intent inference Amiga 68000 subtract
soul	SOUL.txt	Signon file for agents. Reflexes, authority, verification. Read before acting.	SOUL signon agent reflexes authority verification governance subtract
please	PLEASE	Provide Legal Exculpation and Sign Everything. Attestation wallets for clinical 	PLEASE attestation signing clinical call-center exculpation workflow
please-clinical	PLEASE Clinical	Clinical deployment of PLEASE. ABA session logging, HIPAA integrity, insurance b	PLEASE clinical ABA HIPAA session-logging billing insurance 837P
please-economics	PLEASE Economics	Cost model for PLEASE deployments. Hardware, inference, cloud change windows.	PLEASE economics cost hardware inference deployment pricing
boot	Boot	Boot sequence for the formation. What runs, in what order, on which node.	boot sequence formation startup nodes
pattern	Pattern	The pattern subtract follows. Intent to command, file shrinks, adapter learns.	pattern intent command subtract lookdown adapter learning
primitives	Primitives	The kernel primitives subtract depends on. fork, pipe, exec, namespaces.	primitives kernel fork pipe exec namespaces syscall Unix
invariants	Invariants	What does not change. The constraints that hold across all subtract deployments.	invariants constraints subtract deployment rules
subtracting	Subtracting	Why subtract exists. The thesis, lineage, and evidence.	subtracting thesis lineage evidence subtract why
narrow	Narrow	The narrow path. What subtract commits to and what it does not.	narrow scope subtract commitment boundary
signing	signing	signing	signing subtract
substrate	substrate	substrate	substrate subtract
stack-simple	Stack	The subtract dependency stack. What is already there vs what gets added.	stack dependencies kernel shell subtract
discipline	discipline	discipline	discipline subtract
paste	paste	paste	paste subtract
time	time	time	time subtract
why	why	why	why subtract
squared-c	squared.c	Source code for squared.c	squared-c runtime source-code subtract
parsimony-c	parsimony.c	Source code for parsimony.c	parsimony-c runtime source-code subtract
hosuni-sh	hosuni.sh	Source code for hosuni.sh	hosuni-sh runtime source-code subtract
